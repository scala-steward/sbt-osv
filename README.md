...
| `osvReportFormats`         | `Seq[ReportGenerator]` | `Seq(HTML)`   | Report formats to generate. Supported: `ReportGenerator.HTML` (human-readable) and `ReportGenerator.JSON` (machine-readable, for CI pipelines). Set e.g. `Seq(ReportGenerator.HTML, ReportGenerator.JSON)` to emit both.       |
